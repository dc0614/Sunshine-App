...
//  Completed (10) Add constant values to sync Sunshine every 3 - 4 hours
    ...
    private static final int SYNC_HOURS_TO_SECONDS = (int) TimeUnit.HOURS.toSeconds(SYNC_HOURS);
    private static final int SYNC_FLEX_TIME = SYNC_HOURS_TO_SECONDS / 3;
...
//  Completed (12) Create a method to schedule our periodic weather sync
    static void scheduleWeatherSync(@NonNull final Context context){
        ...
        Job syncSunshineJob = dispatcher.newJobBuilder()
                /*Set Service - used to sync data*/
                .setService(SunshineFirebaseJobService.class)
                /*Set Tag - to identify the job*/
                .setTag(SUNSHINE_SYNC_TAG)
                /*Set Constraints - sync over any network*/
                .setConstraints(Constraint.ON_ANY_NETWORK)
                /*Set Lifetime - how long the job should persist*/
                .setLifetime(Lifetime.FOREVER)
                /*Set Recurring - job should recur to keep data in sync*/
                .setRecurring(true)
                /*Set Trigger - data should sync every 3 hours */
                .setTrigger(Trigger.executionWindow(SYNC_HOURS_TO_SECONDS,
                        SYNC_HOURS_TO_SECONDS + SYNC_FLEX_TIME))
                /*Set Replace Current - the new running job will replace an old job*/
                .setReplaceCurrent(true)
                /*Call the build method to return the job*/
                .build();
